...
void TestWindowFilter::testMultipleRules() {
    ...
    WindowDescriptor blocked1{.title = "secret window"};
    WindowDescriptor blocked2{.className = "HiddenClass"};
    WindowDescriptor allowed{.title = "normal", .className = "NormalClass"};

    QVERIFY(!filter.isAllowed(blocked1));
    QVERIFY(!filter.isAllowed(blocked2));
    QVERIFY(filter.isAllowed(allowed));
}
...
void TestWindowFilter::testBuiltinRules() {
    ...
    QVERIFY(rules.rules.size() > 5); // at least 5 builtin rules

    WindowFilter filter;
    filter.setRules(rules);

    WindowDescriptor progman{.className = "Progman"};
    QVERIFY(!filter.isAllowed(progman));
}
